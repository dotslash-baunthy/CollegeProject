...
<body>
	...
	<div class="container">
		...
		<form action="/CollegeProject/books/search" class="form-inline">
		...
		<table class="table table-bordered table-striped">
			...
			<tbody>
				<c:forEach items="${Books}" var="tempBook">
					<tr>
						<td><c:out value="${tempBook.title}" /></td>
						<td><c:out value="${tempBook.category}" /></td>
						<td><c:out value="${tempBook.author}" /></td>
						<td>
							<!-- Add "update" button/link --> <a
							href="/CollegeProject/books/showFormForUpdate?bookId=${tempBook.id}"
							class="btn btn-info btn-sm"> Update </a> <!-- Add "delete" button/link -->
							<a href="/CollegeProject/books/delete?bookId=${tempBook.id}"
